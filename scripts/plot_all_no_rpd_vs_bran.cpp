...
std::string const OUT_FILE_PATH = "../plots/all_no_rpd_vs_bran.root";
 ...
 */
inline void plot_all_no_rpd_vs_bran() {
  ...
  for (auto const& side : SIDES) {
    hHAD1VsBRAN.at(side) = new TH2D(Form("side%c_AllNoRPDVsBRAN", getSideLabel(side)), 
      ";Truth Total Energy (no BRAN or RPD) [MeV];BRAN Truth Energy [MeV];Count", 
      BINS(tot), 
      BINS(axisConfig::BRANTruth.withBins(32)));
  }

  while (reader.Next()) {
    for (auto const& side : SIDES) {
      hHAD1VsBRAN.at(side)->Fill(zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::EM)) + zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::HAD1)) + zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::HAD2)) + zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::HAD3)), zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::BRAN)));
    }
  }

...
inline void plot_had2_vs_bran() {
  ...
  for (auto const& side : SIDES) {
    hHAD1VsBRAN.at(side) = new TH2D(Form("side%c_HAD2VsBRAN", getSideLabel(side)), ";HAD2 Truth Energy [MeV];BRAN Truth Energy [MeV];Count", BINS(axisConfig::EMTruth.withBins(32)), BINS(axisConfig::BRANTruth.withBins(32)));
  }

  while (reader.Next()) {
    for (auto const& side : SIDES) {
      hHAD1VsBRAN.at(side)->Fill(zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::HAD2)), zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::BRAN)));
    }
  }

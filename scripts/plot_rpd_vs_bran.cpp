...
std::string const OUT_FILE_PATH = "../plots/rpd_vs_bran.root";
...
inline void plot_rpd_vs_bran() {
  ...
  for (auto const& side : SIDES) {
    hHAD1VsBRAN.at(side) = new TH2D(Form("side%c_RPDVsBRAN", getSideLabel(side)), ";RPD Truth Energy [MeV];BRAN Truth Energy [MeV];Count", BINS(axisConfig::EMTruth.withBins(32)), BINS(axisConfig::BRANTruth.withBins(32)));
  }

  while (reader.Next()) {
    for (auto const& side : SIDES) {
      hHAD1VsBRAN.at(side)->Fill(zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::RPD)), zdc_ZdcModuleTruthTotal.At(getModuleIndex(side, SimModule::BRAN)));
    }
  }
